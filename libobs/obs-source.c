...
// @xp : obs_source_main_render 调用info.video_render, 终于调用video_render....( ＞﹏＜)。
static inline void obs_source_main_render(obs_source_t *source)
{
	...
	if (default_effect)
		...
		source->info.video_render(source->context.data, // @xp : video_render 在插件结构体中进行了初始化，最后调用到wc_render，然后数据传送到 opengl  或者d3d中进行处理，显示
				custom_draw ? NULL : gs_get_effect());
}

static bool ready_async_frame(obs_source_t *source, uint64_t sys_time);
// @xp : render_video 为了最终调用video_render，调用obs_source_main_render
static inline void render_video(obs_source_t *source)
{
	...
	if (source->filters.num && !source->rendering_filter)
		...
		obs_source_main_render(source);   // @xp : obs_source_main_render 调用info.video_render, 终于调用video_render....( ＞﹏＜)。
...
// @xp : obs_source_video_render 为了最终调用video_render，调用render_video
void obs_source_video_render(obs_source_t *source)
{
	...
	obs_source_addref(source);
	render_video(source);  // @xp : render_video 为了最终调用video_render，调用obs_source_main_render
